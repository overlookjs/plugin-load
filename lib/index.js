...
const {join: pathJoin, sep: pathSep} = require('path'),
	...
	{INIT_PROPS, DEBUG_ERROR} = require('@overlook/route'),
...
const DEFAULT_DIR_INDEX = 'index',
	DEFAULT_ROUTE_EXTS = ['route.js', 'js'];
...
function extend(Route) {
	return class LoadRoute extends Route {
		...
		[INIT_PROPS](props) {
			...
			this[LOAD_PATH] = undefined;
			this[LOAD_DIR_PATH] = undefined;
		}
		...
		async [LOAD_FILE](file, dirPath, isIndex, routes) {
			...
			if (ext != null) {
				// Load route file
				path = exts[ext];
				invariant(path, `ext returned from [IDENTIFY_ROUTE_FILE] '${ext}' is not present for ${path}`);
				route = require(path); // eslint-disable-line global-require, import/no-dynamic-require
			} else if (Class) {
				// Create route from class
				route = new Class();
				path = pathJoin(dirPath, `<${Class.name}>`);
			}

			invariant(route, `Invalid return value from [IDENTIFY_ROUTE_FILE]: ${identified}`);

			// Add props to route
			route.name = file.routeName;
			route[FILES] = exts;
			route[LOAD_PATH] = path;
			route[LOAD_DIR_PATH] = dirPath;
		...
		[ATTACH_FILE](route, dirPath, isIndex, routes) {
			...
			if (parentPath != null) {
				...
				invariant(match, `Invalid parent path '${parentPath}' for '${route[LOAD_PATH]}'`);
				...
				invariant(parent, `Cannot locate parent path '${parentPath}' for '${route[LOAD_PATH]}'`);
			} else {
		...
		async [LOAD_FILES](files, routes) {
			// Load all files
			const dirPath = this[LOAD_DIR_PATH];
			const newRoutes = [];
		...
		[DEBUG_ERROR](err) {
			...
			const loadPath = this[LOAD_PATH];
			if (loadPath) {
				err[LOAD_PATH] = loadPath;
				err.message += ` (load path ${loadPath})`;
			}
			return err;
		}
