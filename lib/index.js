...
module.exports = new Plugin(
	...
	(Route, {
		...
		async [LOAD_DIR](dirPath, dirName, routes) {
			...
			if (!file) {
				file = {name: indexName, routeName: dirName, exts: {}};
			} else {
		...
		async [LOAD_FILE](file, dirPath, isIndex, routes) {
			...
			if (isString(identified)) {
				...
				assert(path, `ext returned from [IDENTIFY_ROUTE_FILE] '${identified}' is not present`);
				route = await loadFile(path);
				assert(isRoute(route), `file at '${path}' located via [IDENTIFY_ROUTE_FILE] is not a Route`);
