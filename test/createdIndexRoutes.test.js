...
let Route, SRC_PATH, SRC_DIR_PATH, SRC_FILENAME, PARENT_PATH;
beforeEach(() => {
	({Route} = modules);
	({SRC_PATH, SRC_DIR_PATH, PARENT_PATH, SRC_FILENAME} = modules.loadPlugin);
});
...
function describeTests(fixtureName) {
	...
	beforeEach(async () => {
		fixturePath = getFixturePath('created index routes', fixtureName);
	...
	describe('subdirectory (created index)', () => {
		let route;
		...
		it('was created from specified class', () => {
			expect(route._isIndexRoute).toBeTrue();
		});
	...
	describe('2nd level subdirectory (created index)', () => {
		let route;
